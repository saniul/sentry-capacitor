...
const jsonFilter = /\s*\"\@sentry\/(?!capacitor|wizard|cli|typescript|electron|vite-plugin|nextjs)(?<packageName>[a-zA-Z]+)\"\:\s*\"(?<version>.+)\"/;
...
function ValidateSentryPackageParameters(packages) {
  let errorMessages = [];
  var packageFilter = /.*(capacitor|cli|wizard|typescript|vite-plugin|nextjs)/;
